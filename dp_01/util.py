...
def calc_s21_metrics(f, db, provided_insertion_loss=None):
    insertion_loss = np.max(db)
    max_gain_freq = f[find_nearest_idx(db, insertion_loss)]
    
    if not provided_insertion_loss:
        provided_insertion_loss = insertion_loss

    passband_edge = f[find_nearest_idx(db, -provided_insertion_loss-3)]
    stopband_start = f[find_nearest_idx(db, -provided_insertion_loss-20)]
...
def plot_s21_dc_stop_metrics(ax, metrics):
    offset_bars = 0.03
    ...
    ax.scatter([metrics['max_gain_freq'], metrics['max_gain_freq']], [0+offset_bars, -metrics['insertion_loss']-offset_bars], color='green', marker="_", s=100, zorder=5)
...
def plot_s21_pass_metrics(ax, metrics, insertion_loss):
    offset_bars = 0.03

    ax.vlines(metrics['max_gain_freq'], 0, -metrics['insertion_loss'], label=f"Minimum Ripple: {metrics['insertion_loss']:.2f} dB", colors='green', linewidth=3)
    ax.scatter([metrics['max_gain_freq'], metrics['max_gain_freq']], [0+offset_bars, -metrics['insertion_loss']-offset_bars], color='green', marker="_", s=100, zorder=5)

    ax.vlines(metrics['worst_ripple_freq'], 0+offset_bars, -metrics['insertion_loss']-metrics['inband_ripple'], label=f"Maximum Ripple: {metrics['insertion_loss']+metrics['inband_ripple']:.2f} dB", colors='orange', linewidth=3)
    ax.scatter([metrics['worst_ripple_freq'], metrics['worst_ripple_freq']], [0+offset_bars, -metrics['insertion_loss']-metrics['inband_ripple']-offset_bars], color='orange', marker="_", s=50, zorder=5)

    ax.scatter(metrics['passband_edge'], -insertion_loss-3, label=f"Pass band edge: {metrics['passband_edge']/1e6:.2f} MHz", c='red', zorder=10)
...
# def plot_s21_metrics(ax, metrics, skip_ripple=False, skip_stopband=True, ripple_str="Insertion Loss"):
#     offset_bars = 0.01

#     ax.vlines(metrics['max_gain_freq'], 0, -metrics['insertion_loss'], label=f"{ripple_str}: {metrics['insertion_loss']:.2f} dB", colors='green', linewidth=3)
#     ax.scatter([metrics['max_gain_freq'], metrics['max_gain_freq']], [0+offset_bars, -metrics['insertion_loss']-offset_bars], color='green', marker="_", s=50, zorder=5)
...
#         ax.scatter([metrics['worst_ripple_freq'], metrics['worst_ripple_freq']], [-metrics['insertion_loss']+offset_bars, -metrics['insertion_loss']-metrics['inband_ripple']-offset_bars], color='orange', marker="_", s=50, zorder=5)
